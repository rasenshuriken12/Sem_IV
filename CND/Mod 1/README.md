...
- converts plaintext into an ciphertext before sending it. At the receiver side, decryption,
...
- Establishment: Before data exchange starts, the session layer creates a connection between two applications.
- Maintenance: While communication continues, the session layer keeps the session active and manages data exchange.
- Termination: After communication is completed, the session layer properly closes the connection.
